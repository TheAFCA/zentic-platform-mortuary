...
describe('TenantModuleGuard', () => {
  ...
  beforeEach(() => {
    ...
    guard = new TenantModuleGuard(reflector, prisma as unknown as PrismaService);
  });
  ...
  it('returns false when user is missing', async () => {
    reflector.getAllAndOverride.mockReturnValueOnce(false).mockReturnValueOnce('leads');
  ...
  it('allows a non-impersonating super admin without checking the flag', async () => {
    reflector.getAllAndOverride.mockReturnValueOnce(false).mockReturnValueOnce('leads');
    const request = { user: { role: UserRole.SUPER_ADMIN }, tenantId: 'tenant-123' };
  ...
  it('does NOT bypass for TENANT_ADMIN (unlike PermissionGuard)', async () => {
    ...
    const request = { user: { role: UserRole.TENANT_ADMIN, tenantId: 'tenant-123' }, tenantId: 'tenant-123' };
  ...
  it('allows when the flag exists and is enabled', async () => {
    ...
    const request = { user: { role: UserRole.OPERATOR, tenantId: 'tenant-123' }, tenantId: 'tenant-123' };
